...
class OgCommand {
    ...
    func byte2Int(_ array:[UInt8]) -> Int {
        print(OgCommand.bytesToHex(array))
    ...
    func GetPr()-> [SensorData] {
        ...
        while (true) {
            ...
            if (Command.timer.stop() > 20 || Command.rx == GetCrcString("F51C000301000A") ||  Command.rx == GetCrcString("F51C000302000A") || cancel) {
                if (Command.timer.stop() > 20) {
                    ReOpen()
                }
                return response
    ...
    func SendTrigerInfo(sensor: [String])->Bool {
        for i in 0..<sensor.count {
            ...
            while (Rx.length != 36) {
                ...
                if (Command.timer.stop() > 20 || Rx == GetCrcString("F51C000301000A") || Rx == GetCrcString("F51C000302000A") || cancel) {
    ...
    func ProgramFirst(Lf: String, Hex: String, count: String, data: String) -> Bool {
        ...
        var fal = 1
        while (true) {
            if (Rx == GetCrcString("F51C000301000A") || Rx == GetCrcString("F51C000302000A") || cancel) {
                if (fal == 3 || cancel) {
    ...
    func CheckData(_ data: String,_ place: String)-> Bool {
        ...
        let fal = 0
        while (true) {
            if (Command.timer.stop() > 6 || Rx == GetCrcString("F51C000301000A") || Rx == GetCrcString("F51C000302000A") || cancel ) {
...
//0A02002B003533333133313333333733303330333033303330333333303330333033323330343333393337333920F5
//    0A02002B005331313337303030303033303030323043393739303030303631373930303030363137393030303031F5
    ...
    public  func askVersion()->Bool{
        if(Command.goState(BootloaderState.Bootloader)){
                print("跳轉成功")
            ...
            Command.timer.zeroing()
                while (true) {
                    ...
                    if(Command.rx.count>=54){
                        ...
                        print("ogversion",Command.rx.sub(8..<20))
                        return true
                    }
                    usleep(1000*100)
                }
    ...
    func IdCopy(_ caller: (_ a:Bool)->Void,_ obd:[Md_Idcopy],_ copyNext:(_ a:Bool,_ position:Int)->Void) {
        ...
        for i in 0..<obd.count{
            ...
            Command.timer.zeroing()
            while (true) {
                if (Command.timer.stop() > 15 || Command.rx == GetCrcString("F51C000301000A") || Command.rx == GetCrcString("F51C000302000A") || cancel) {
                    if (Command.timer.stop() > 15) {
                        ReOpen()
                        caller(false)
                        return
...
//    0A02002B015331313337303130303030303030303030303030303030303030303030303030303030303030303044F5
    
//    0A02002B02533131333730323030303030303030303030303030303030303030303030303036313739303030304D
    ...
    func goBootloader()->Bool{
      ...
      var fal=0
        while(true){
            if(Command.timer.stop() > 10 || cancel){
                fal+=1
                if(fal==1){
                    return false
                }
